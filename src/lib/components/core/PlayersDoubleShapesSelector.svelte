<script lang="ts">
	...
	let { resetEnabled, shapes = [] }: Props = $props();

	let selectedConfiguration: DoubledPlayers = $state(null);
	...
	function selectOption(option: DoubledPlayers) {
		selectedConfiguration =
			selectedConfiguration === option && !optionDisabled(option) ? null : option;
	...
	<div class="options-container">
		{#each options as option}
			<label
				...
				class:selected={selectedConfiguration == option.value && !optionDisabled(option.value)}
