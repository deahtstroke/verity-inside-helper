<script lang="ts">
	...
	import { strategyModeStore } from '../stores/StrategyModeStore';

	const FAST_STRATEGY_ENABLED: string = 'fast-strategy-enabled';

	// default is normal-mode
	let isFastStrategy: boolean = $state(false);
	...
	onMount(() => {
		if (browser) {
			...
			position.target = isFastStrategy ? 1 : 0;
...
<button class="strategy-slider" onclick={toggleStrategy} aria-label="Toggle strategy mode">
	<div class="slider-track">
		<div class="slider-thumb" style="transform: translateX({position.target * 1.5}rem)">
			{#if isFastStrategy}
				<Zap size={16} />
			{:else}
				<Turtle size={16} />
